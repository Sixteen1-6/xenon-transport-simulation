...
# 1. SMILES to 3D geometry using RDKit
    ...
    params.randomSeed = 1  # deterministic embedding for reproducibility (optional)
    ...
    if embed_status != 0:
        ...
        sys.exit(1)
    # Optional: optimize geometry with MMFF to relieve bad contacts
    ...
    mf.conv_tol = 1e-9  # tighten convergence if needed for accuracy
    mf.verbose = 0      # set verbose to >0 to see SCF output
    mf.kernel()         # run HF SCF

    # 3. Geometry optimization using PySCF's internal optimizer (PyBerny backend)
    try:
        mol_eq = optimize(mf, maxsteps=100)  # returns optimized Mole object
    except Exception as e:
        sys.stderr.write(f"Optimization failed: {e}\n")
        sys.exit(1)
    # After optimization, perform a final HF on optimized geometry to get wavefunction at optimum
    mf_eq = scf.RHF(mol_eq).run(conv_tol=1e-9, verbose=0)

    # 4. Compute Hessian (analytic second derivatives) at optimized geometry&#8203;:contentReference[oaicite:1]{index=1}
    hessian = mf_eq.Hessian().kernel()  # 4D array: (natm, natm, 3, 3)

    # 5. Vibrational frequency analysis (for verification)
    ...
    # Only take non-negative frequencies (imaginary frequencies appear as negative values)
    ...
    def internal_force_constant(v_vec):
        ...
        v_vec is an array of shape (natm, 3) representing ∂(Cartesians)/∂(internal coordinate)."""
        k = 0.0
        # Sum_{i,j,α,β} v[i,α] * Hessian[i,j,α,β] * v[j,β]
        for i in range(natm):
            for j in range(natm):
                # dot over Cartesian components
                # (This double loop is efficient enough for moderate natm, as complexity ~ O(natm^2))
                k += np.dot(v_vec[i], hessian[i, j].dot(v_vec[j]))
        return k

    # 6. Extract and print optimized bond lengths and bond angles
    # Use RDKit's connectivity (bonds) and neighbor relationships from the original molecule
    ...
    for bond in mol.GetBonds():
        ...
        # move atoms i and j by ±0.5 along the bond direction
        v = np.zeros((natm, 3))
        v[i] -= 0.5 * u
        v[j] += 0.5 * u
    ...
    # For each bond angle (i-center-k), compute force constant for that angle coordinate
    # Use similar approach: small rotation of bonds i-center and k-center in opposite directions
    for (i, j, k, angle_deg) in angles_list:
        ...
        # If either vector is near zero (shouldn't happen if bonded), skip
        if np.linalg.norm(v1) < 1e-8 or np.linalg.norm(v2) < 1e-8:
            continue
        # Unit normal to plane of v1 and v2 (or an arbitrary perpendicular if angle ~ 180°)
        n = np.cross(v1, v2)
        if np.linalg.norm(n) < 1e-8:
            # For linear or nearly linear angles, choose an arbitrary perpendicular direction for bending
            # e.g., take cross with an arbitrary axis (here X-axis) to define a pseudo-normal
        ...
        # Compute displacements: rotate i and k around n in opposite directions by a small angle (half each)
        # Use differential: ∂r_i = 0.5 * (n_unit × v1), ∂r_k = -0.5 * (n_unit × v2), ∂r_center = 0
        disp_i = 0.5 * np.cross(n_unit, v1)
        disp_k = 0.5 * np.cross(n_unit, v2)
        v = np.zeros((natm, 3))
        v[i] += disp_i
        v[k] += disp_k
        # (center atom j has no displacement for pure angle bend coordinate)
...
# We'll do partial constraints by forcibly rescaling O–H to no more than ~0.1 nm
# and forcibly adjusting angles if they exceed some threshold from eq.
...
# 0=O, 1=H, 2=Xe, 3=Zn
# Water indexing: for water w, O=3w, H1=3w+1, H2=3w+2
...
image_folder = '/Users/Screen_share_account/code_for_pchem_calcuations/hackathon/frames'
fps = 24  # Frames per second

image_files = sorted([os.path.join(image_folder, img) for img in os.listdir(image_folder) if img.endswith(".png")])
clip = ImageSequenceClip(image_files, fps=fps)
clip.write_videofile('large_sim.mp4', fps=fps)
